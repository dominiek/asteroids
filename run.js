...
function playBackgroundMusic() {
    ...
    const track = new Audio('assets/sounds/' + id + '.mp3')
...
function playSounds(type) {
    if (type === 'fire') {
        new Audio('assets/sounds/shoot.wav').play()
    }
    if (type === 'atmosphere') {
        new Audio('assets/sounds/atmosphere.wav').play()
    }
    if (type === 'engine') {
        new Audio('assets/sounds/rattle.wav').play()
    }
    if (type === 'explosion') {
        const id = Math.floor(Math.random() * 4) + 1
        new Audio('assets/sounds/explosion'+id+'.wav').play()
    }
}
